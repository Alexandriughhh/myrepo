...
# Overtraining and Over smoothing Code
...
#Nearest Neighbors. What is the max value of F_1?
...
data(heights)
index <- createDataPartition(heights$sex, p = 0.5, list = FALSE)
train_set <- heights[index, ]
test_set <- heights[-index, ]

# Define predictor and outcome
train_x <- train_set$height
train_y <- train_set$sex
test_x <- test_set$height
test_y <- test_set$sex
...
F1_scores <- sapply(ks, function(k) {
  y_hat <- knn(train = matrix(train_x), test = matrix(test_x), cl = train_y, k = k)
...
# Find the k that gives the max F1 score
best_k <- ks[which.max(F1_scores)]
best_k
ks[which.max(F1_scores)]

#predicting tissue type using KNN
